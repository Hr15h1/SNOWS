    ...
    for name, param in model.named_parameters():
        if name in params:
            ...
            if isinstance(w, torch.Tensor):
                new_values = w[pointer: pointer + count]
            else:
                state_dict = torch.tensor(w[pointer: pointer + count], device = device)

            param.copy_(new_values.view_as(param))
            pointer += count
...
def get_vit_b_16_cifar10(pretrained=False, dset_path='./data', model_path='./checkpoints/vit_model_cifar10_2.pth'):
    ...
    feature_extractor = AutoImageProcessor.from_pretrained('aaraki/vit-base-patch16-224-in21k-finetuned-cifar10')
