...
def hessian_vector_product_chunks(grad_W, W, vector, mask, max_chunk_size=5e4):
    ...
    num_elements = masked_indices.numel()

    if num_elements == 0:
        return torch.zeros(0, device=device)

    for i in range(0, num_elements, max_chunk_size):
        end = min(i + max_chunk_size, num_elements)
        chunk_indices = masked_indices[i:end]

        chunk_mask = torch.zeros_like(W, dtype=torch.bool, device=device)
        chunk_mask.view(-1)[chunk_indices] = True
        ...
        hvp.view(-1)[chunk_indices] = chunk_hvp.view(-1)[chunk_indices]
...
def hessian_vector_product(grad_W, W, vector, mask):
    device = W.device
    vector = vector.to(device).requires_grad_(True)

    full_vector = torch.zeros_like(W, device=device)
    full_vector[mask] = vector

    dot = (grad_W * full_vector).sum()

    hvp = torch.autograd.grad(dot, W, retain_graph=False, create_graph=False)[0]
